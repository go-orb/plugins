...
var (
	...
	ErrClosed = errors.New("drpc pool: client pool is closed")
	// ErrTimeout is the error when the client pool timed out.
	ErrTimeout = errors.New("drpc pool: client pool timed out")
	// ErrAlreadyClosed is the error when the client conn was already closed.
	ErrAlreadyClosed = errors.New("drpc pool: the connection was already closed")
	// ErrFullPool is the error when the pool is already full.
	ErrFullPool = errors.New("drpc pool: closing a ClientConn into a full pool")
)

// FactoryWithContext is a function type creating a drpc client
// that accepts the context parameter that could be passed from
// Get or NewWithContext method.
type FactoryWithContext func(ctx context.Context, addr string, tlsConfig *tls.Config) (*drpcconn.Conn, error)

// Pool is the drpc client pool.
type Pool struct {
	capacity        int
...
// ClientConn is the wrapper for a drpc conn.
...
func (p *Pool) Get(ctx context.Context, addr string, tlsConfig *tls.Config) (*ClientConn, error) {
	...
	if wrapper.Conn != nil && idleTimeout > 0 && wrapper.timeUsed.Add(idleTimeout).Before(time.Now()) {
		wrapper.Conn.Close() //nolint:errcheck,gosec
		wrapper.Conn = nil
	}

	var err error
	if wrapper.Conn == nil {
		wrapper.Conn, err = p.factory(ctx, addr, tlsConfig)
		if err != nil {
