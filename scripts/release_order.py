...
def collect_release_order(graph, node, visited=None, release_order=None):
    """Collect modules in release order (deepest dependencies first)."""
    if visited is None:
        visited = set()
    ...
    for dep in graph.get(node, []):
        collect_release_order(graph, dep, visited, release_order)
    
    # Then add current node
    if node not in release_order:
        release_order.append(node)
...
def main():
    ...
    for root in sorted(roots):
        release_order.extend(
            module for module in collect_release_order(dependency_graph, root)
            if module not in release_order
        )

    for module in release_order:
        if module.startswith('github.com/go-orb/plugins/'):
            print(module.replace('github.com/go-orb/plugins/', ''))
