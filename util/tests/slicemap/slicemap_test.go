	...
	for i, test := range sliceTests {
		t.Run("SliceTest"+strconv.Itoa(i), func(t *testing.T) {
			for q, query := range test.Query {
				assert.Equal(t, slicemap.In(test.Array, query), test.Expected[q])
			}
		})
	}
}
...
func TestLookup(t *testing.T) {
	for i, test := range mapTests {
		t.Run("MapTest"+strconv.Itoa(i), func(t *testing.T) {
			for q, query := range test.Query {
				v, err := slicemap.Lookup(test.Map, query)
...
func TestSetValue(t *testing.T) {
	for i, test := range setValueTests {
		t.Run("SetValueTest"+strconv.Itoa(i), func(t *testing.T) {
			slicemap.SetValue(test.Input, test.Path, test.Value)
...
func TestGet(t *testing.T) {
	...
	val, ok := slicemap.Get[int](a, "one")
	assert.True(t, ok)
	assert.Equal(t, 5, val)

	_, ok = slicemap.Get[string](a, "two")
	assert.False(t, ok)

	_, ok = slicemap.Get[string](a, "three")
	assert.False(t, ok)
}
